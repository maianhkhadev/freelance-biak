  ...
  export default {
    ...
    methods: {
      ...
      save: function () {
        ...
        axios.post(`/articles/${self.data.id}`, formData).then(res => {
          if (res.status === 200) {
    ...
    mounted () {
      ...
      axios.get(`/articles/${id}/edit`).then(res => {
        self.collections = res.data.collections

        self.data.id = res.data.article.id
        self.data.title = res.data.article.title
        self.data.description = res.data.article.description
        self.data.collection_id = res.data.article.collection_id
        self.data.status = res.data.article.status

        let content = JSON.parse(res.data.article.content)
        self.$refs.editor.setContents(content)

        var thumbnail = document.querySelector('.thumbnail')
        thumbnail.style['background-image'] = `url(${res.data.article.thumbnail_url})`
      })
